...
class PetitWebGame extends StatefulWidget {
  const PetitWebGame({
    super.key,
  });

  @override
  State<PetitWebGame> createState() => _PetitWebGamePageState();
}

class _PetitWebGamePageState extends State<PetitWebGame> {
  ...
  void initState() {
    ...
    final petitIndices = List<int>.generate(petit.length, (i) => i);
    randompetit = petitIndices.map((index) => petit[index]).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    cards = randompetit
        .map((gameContents) =>
            GameCard(gameContents: gameContents, fontSize: width * 0.12))
        .toList();
    return Scaffold(
